...
# Create undersampling model
rus = RandomUnderSampler(random_state=123)
X_resampled, y_resampled = rus.fit_resample(X_train, y_train) # Fit undersampler with training data

# Estimators == number of trees made
# max_features == number of features to consider (default is total num)
estimators = 50
max_features = 6

#Create model
rf = RandomForestClassifier(n_estimators=estimators, random_state=123)
rf.fit(X_resampled, y_resampled)# Fit model with undersampled data

y_pred = rf.predict(X_validation) #predict validation data set
y_testPred = rf.predict(X_test) # predict test data
...
plt.savefig('RFCconfMatrixundersampled.png') # save to files
...
plt.savefig("rfcundersampledprc") # save to files
...
print("---------------------------------------")
print(f"Score for validation data:{rf.score(X_validation, y_validation)}")
print("---------------------------------------")

...
X_resampled, y_resampled = rus.fit_resample(X_train, y_train) # perform under sampling on training data

#Define model using regularization 0.1
svm = LinearSVC(C=0.1, random_state=123)
svm.fit(X_resampled, y_resampled)#fit model to resampled data
...
plt.savefig('SVCconfMatrixundersampled.png')#save to files
...
plt.savefig("svcundersampledprc") # save to files

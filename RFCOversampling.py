...
# Apply SMOTE to data - 1.0 sampling strat means min and maj categories have same number of samples
...
rf.fit(X_resampled, y_resampled) # Fit model with oversampled data

y_pred = rf.predict(X_validation) # Predict with validation data
y_testPred = rf.predict(X_test) #Predict with test data
...
plt.savefig('RFCconfMatrixoversampled.png') #save to files
...
plt.savefig("RVCoversampledprc") #save to files
